...
{
    { COMPOUND_STRING("获得某个道具…"),    DebugAction_Selection_Init, &sGiveItemSelection },
    { COMPOUND_STRING("获得宝可梦(简易)"),   DebugAction_Selection_Init, &sSimplePokemonSelection },
    { COMPOUND_STRING("获得宝可梦(详细)"), DebugAction_Selection_Init, &sComplexPokemonSelection },
    { COMPOUND_STRING("获得宝可梦蛋"),          DebugAction_Selection_Init, &sEggPokemonSelection },
    { COMPOUND_STRING("获得装饰物品…"),  DebugAction_Selection_Init, &sGiveDecorationSelection },
...
{
    { COMPOUND_STRING("从地图选择训练家"), DebugAction_Selection_Init, &sTrainerFromMapSelection},
    { COMPOUND_STRING("训练家 1: {STR_VAR_1}"), DebugAction_Selection_Init, &sTrainer1Selection},
    { COMPOUND_STRING("训练家 2: {STR_VAR_1}"), DebugAction_Selection_Init, &sTrainer2Selection},
    { COMPOUND_STRING("搭档: {STR_VAR_1}"), DebugAction_Selection_Init, &sPartnerSelection},
...
{
    { COMPOUND_STRING("设置固定大量出现"), DebugAction_Selection_Init, &sStaticMassOutbreakSelection },
    { COMPOUND_STRING("清除当前大量出现"), DebugAction_Outbreak_ClearActive },
    { COMPOUND_STRING("宝可梦: {STR_VAR_1}"), DebugAction_Selection_Init, &sMassOutbreakSpeciesSelection },
    { COMPOUND_STRING("地图: {STR_VAR_1}"), DebugAction_Selection_Init, &sMassOutbreakLocationSelection },
    { COMPOUND_STRING("等级: {STR_VAR_1}"), DebugAction_Selection_Init, &sMassOutbreakLevelSelection },
    { COMPOUND_STRING("招式"), DebugAction_Selection_Init, &sMassOutbreakMovesSelection },
    { COMPOUND_STRING("概率: {STR_VAR_1}"), DebugAction_Selection_Init, &sMassOutbreakProbabilitySelection },
    { COMPOUND_STRING("剩余天数: {STR_VAR_1}"), DebugAction_Selection_Init, &sMassOutbreakDaysLeftSelection },
    { COMPOUND_STRING("设置动态大量出现"), DebugAction_Selection_Init, &sDynamicMassOutbreakSelection },
    { NULL }
};

static const struct DebugMenuOption sDebugMenu_Actions_Encounters[] =
{
    { COMPOUND_STRING("大量出现…"), DebugAction_OpenOutbreakMenu, sDebugMenu_Actions_MassOutbreak },
    { NULL }
};
...
{
    [DEBUG_FLAGVAR_MENU_ITEM_FLAGS]                = { COMPOUND_STRING("设定某个标志…"),                     DebugAction_Selection_Init, &sToggleFlagSelection },
    [DEBUG_FLAGVAR_MENU_ITEM_VARS]                 = { COMPOUND_STRING("设定某个变量…"),                      DebugAction_Selection_Init, &sSetVarSelection},
...
static void DebugSelectionStep_UpdateMapGroup(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("组别: {STR_VAR_2} / {STR_VAR_3}"));
...
static void DebugSelectionStep_UpdateMapNum(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    ConvertIntToDecimalStringN(gStringVar2, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    ConvertIntToDecimalStringN(gStringVar3, max, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("地图: {STR_VAR_2} / {STR_VAR_3}"));
    StringCopy(gStringVar2, COMPOUND_STRING("名称:"));
...
static void DebugSelectionStep_UpdateWarp(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    ConvertIntToDecimalStringN(gStringVar2, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    ConvertIntToDecimalStringN(gStringVar3, max, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("出口: {STR_VAR_2} / {STR_VAR_3}"));
...
static void DebugSelectionStep_UpdateWeather(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("天气ID: {STR_VAR_3}"));
...
static void DebugSelectionStep_UpdateGeneratorArgs(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    ConvertIntToDecimalStringN(gStringVar3, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("参数{STR_VAR_2}: {STR_VAR_3}"));
    StringCopy(gStringVar2, COMPOUND_STRING(""));
    StringCopy(gStringVar3, COMPOUND_STRING(""));
    DebugNativeStep_PrintWindowSelection(taskId);
}
...
{
    ConvertIntToDecimalStringN(gStringVar2, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("选项ID: {STR_VAR_2}"));
    StringCopy(gStringVar2, COMPOUND_STRING(""));
    StringCopy(gStringVar3, COMPOUND_STRING(""));
    DebugNativeStep_PrintWindowSelection(taskId);
}
...
static void DebugSelectionStep_UpdateMapTrainer(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    ConvertIntToDecimalStringN(gStringVar3, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("本地ID: {STR_VAR_3}"));
    u8 *end;
    if (trainerId == TRAINER_NONE)
        end = StringCopy(gStringVar2, COMPOUND_STRING("不是训练家"));
    else
...
static void DebugSelectionStep_PrepareTrainerDisplay(u8 taskId, u8 digits, u32 trainerId)
{
    ...
    if (gTasks[taskId].tInput == TRAINER_NONE )
        end = StringCopy(gStringVar2, COMPOUND_STRING("无"));
    else
        end = StringCopy(gStringVar2, GetTrainerNameFromId(trainerId));
    WrapFontIdToFit(gStringVar2, end, DEBUG_MENU_FONT, WindowWidthPx(gTasks[taskId].tSubWindowId));
    StringCopy(gStringVar3, COMPOUND_STRING(""));
...
static void DebugSelectionStep_PrintGenericBooleanInput(u8 taskId, const u8 *str)
{
    ...
    if (gTasks[taskId].tInput == 0)
        StringCopy(gStringVar2, COMPOUND_STRING("否"));
    else
        StringCopy(gStringVar2, COMPOUND_STRING("是"));
    StringCopy(gStringVar3, COMPOUND_STRING(""));
    DebugNativeStep_PrintWindowSelection(taskId);
}
...
static void DebugSelectionStep_UpdateSpecies(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("宝可梦: {STR_VAR_3}"));
...
static void DebugSelectionStep_UpdateMoves(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("招式{STR_VAR_2}: {STR_VAR_3}"));
    u8 *end;
    if (moveId == MOVES_COUNT)
        end = StringCopy(gStringVar2, COMPOUND_STRING("默认"));
    else
...
UPDATE_GENERIC_INPUT(Level, 等级)
UPDATE_GENERIC_INPUT(OutbreakProbability, 概率)
UPDATE_GENERIC_INPUT(OutbreakDaysLeft, 剩余天数)
...
{
    u8 *end;
    end = StringCopy(gStringVar1, COMPOUND_STRING("变量: 0x"));
    ConvertIntToHexStringN(end, varId, STR_CONV_MODE_LEFT_ALIGN, 4);
    end = StringCopy(gStringVar2, COMPOUND_STRING("值: "));
...
static void DebugSelectionStep_UpdateVarValue(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    end = StringCopy(gStringVar3, COMPOUND_STRING("新值: "));
...
static void DebugSelectionStep_UpdateItem(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("道具ID: {STR_VAR_3}"));
...
static void DebugSelectionStep_UpdateGender(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    if (gender == MON_FEMALE)
        StringCopy(gStringVar1, COMPOUND_STRING("雌性"));
    else if (gender == MON_MALE)
        StringCopy(gStringVar1, COMPOUND_STRING("雄性"));
    else
        StringCopy(gStringVar1, COMPOUND_STRING("无性别"));
    ConvertIntToDecimalStringN(gStringVar3, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringExpandPlaceholders(gStringVar2, COMPOUND_STRING("{STR_VAR_1} ({STR_VAR_3})"));
    StringCopy(gStringVar1, COMPOUND_STRING("性别值:"));
    StringCopy(gStringVar3, COMPOUND_STRING(""));
    DebugNativeStep_PrintWindowSelection(taskId);
}
...
static void DebugSelectionStep_UpdateAbility(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    ConvertIntToDecimalStringN(gStringVar3, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringCopy(gStringVar1, COMPOUND_STRING("特性:"));
...
{
    ConvertIntToDecimalStringN(gStringVar3, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("性格: {STR_VAR_3}"));
...
static void DebugSelectionStep_UpdateIVs(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("{STR_VAR_3}个体值:"));
    ConvertIntToDecimalStringN(gStringVar2, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringCopy(gStringVar3, COMPOUND_STRING(""));
    DebugNativeStep_PrintWindowSelection(taskId);
}
...
{
    StringCopy(gStringVar3, gStatNamesTable[gTasks[taskId].tSubstep]);
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("{STR_VAR_3}努力值:"));
    ConvertIntToDecimalStringN(gStringVar2, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringCopy(gStringVar3, COMPOUND_STRING(""));
    DebugNativeStep_PrintWindowSelection(taskId);
}
...
{
    DebugSelectionStep_PrintGenericInput(taskId, digits, COMPOUND_STRING("极巨化等级:"));
}

static void DebugSelectionStep_UpdateGigantamaxFactor(u8 taskId, u8 digits, u32 min, u32 max)
{
    DebugSelectionStep_PrintGenericBooleanInput(taskId, COMPOUND_STRING("超极巨化:"));
}
...
static void DebugSelectionStep_UpdateTeraType(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    ConvertIntToDecimalStringN(gStringVar3, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("太晶属性: {STR_VAR_3}"));
...
static void DebugSelectionStep_UpdateDecoration(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    StringExpandPlaceholders(gStringVar1, COMPOUND_STRING("装饰物品ID: {STR_VAR_3}"));
...
static void DebugSelectionStep_UpdateFriendship(u8 taskId, u8 digits, u32 min, u32 max)
{
    ...
    StringCopy(gStringVar1, COMPOUND_STRING("亲密度:"));
    StringCopy(gStringVar3, COMPOUND_STRING(""));
    DebugNativeStep_PrintWindowSelection(taskId);
}
...
{
    StringCopy(gStringVar1, COMPOUND_STRING("病毒株:"));
    ConvertIntToDecimalStringN(gStringVar2, gTasks[taskId].tInput, STR_CONV_MODE_LEADING_ZEROS, digits);
    StringCopy(gStringVar3, COMPOUND_STRING(""));
    DebugNativeStep_PrintWindowSelection(taskId);
}
...
{
    StringCopy(gStringVar1, COMPOUND_STRING("剩余天数:"));
    ...
    if (daysLeft == 0 && strain == 0)
        StringCopy(gStringVar3, COMPOUND_STRING("未感染"));
    else if (daysLeft == 0)
        StringCopy(gStringVar3, COMPOUND_STRING("已免疫"));
    else
        StringCopy(gStringVar3, COMPOUND_STRING(""));
    DebugNativeStep_PrintWindowSelection(taskId);
}
